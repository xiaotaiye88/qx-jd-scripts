...
- 打开 App 抓包时，脚本按 `smzdm_id` **自动去重合并**到 BoxJs 的 `smzdm_cookies`（JSON 数组），多账号轮流打开 App 即可全部收录，通知会提示「共 N 个账号」。
- 签到任务会**循环所有账号**依次执行，一次通知汇总所有结果（每个账号前带昵称标签）。
- 也可在 BoxJs → 什么值得买签到 → `smzdm_cookies` 手动编辑（格式 `[{"cookie":"sess=xxx;...","name":"昵称"}]`）。
- 旧版单键 `smzdm_cookie` 会自动迁移到 `smzdm_cookies`，无需手动处理。
